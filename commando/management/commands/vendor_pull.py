    ...
    def handle(self, *args: Any, **options: Any):
        ...
        for name, url in VENDOR_STATICFILES.items():
            ...
            if dl_success:
                ...
                self.stdout.write(
                    self.style.ERROR(f"Download falhou {url}")
                )
        if set(completed_urls) == set(VENDOR_STATICFILES.values()):
            self.stdout.write(
                self.style.SUCCESS('Arquivos estáticos do vendor atualizados com sucesso')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS('Alguns arquivos não foram atualizados com sucesso')
            )